...
with open(output_file, 'w') as file:
    for cycle in range(num_cycles):
        ...
        for i in range(shut_days):
            day_number = cycle * (2 * deltat + shut_days) + deltat + i + 1
            file.write(f"-- Shut-in day {day_number}:\n\n")
            file.write(f"WCONINJE\n\t'BOTTOM'\tWAT\t'SHUT'\tRATE\t0\t1*\t{IBHP}/\n/\n")
            file.write(f"WCONPROD\n\t'TOP'\t'SHUT'\t'WRAT'\t1*\t0\t1*\t1*\t1*\t{PBHP}/\n/\n")
            file.write(f"TSTEP\n  1.0 /\n\n")
        ...
        for i in range(deltat):
            day_number = cycle * (2 * deltat + shut_days) + deltat + shut_days + i + 1
